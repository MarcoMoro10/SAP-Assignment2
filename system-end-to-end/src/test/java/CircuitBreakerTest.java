 ...
 * ({@code account_circuit_open == 1}). Registration is the gateway->account path guarded by the breaker
 * (authentication now flows through the session-service). The account-service is reconnected afterwards
 * (and the return to 0 is checked) so the system is restored for any other test.
 */
